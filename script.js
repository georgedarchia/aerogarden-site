...
async function renderChefOrders() {
    ...
    myOrders.forEach(o => {
        ...
        html += '<div class="order-card ' + (o.status === 'cancelled' ? 'order-cancelled' : '') + '">' +
            ...
            '<div class="delivery-badge">🚚 ჩაბარების თარიღი: ' + o.delivery_date + '</div><br>' + statusBadge + '</div>' +
...
async function renderAdminOrders() {
    ...
    orders.forEach(o => {
        ...
        html += '<div class="order-card ' + (o.status === 'cancelled' ? 'order-cancelled' : '') + '">' +
            '<div class="order-header">' +
            '<div><strong>' + (o.restaurant || 'რესტორანი') + '</strong> (' + (o.chef_name || 'შეფი') + ')<br><small>📞 ' + (o.phone || '-') + ' | 📅 შეკვეთის თარიღი: ' + o.date_str + '</small><br><div class="delivery-badge">🚚 ჩაბარების თარიღი: ' + o.delivery_date + '</div><br>' + statusBadge + '</div>' +
            '<div style="color:#69bf4a; font-weight:bold;">' + o.id + '</div>' +
            '</div>' +
            '<ul class="order-items">' + itemsHtml + '</ul>' +
            '<div class="order-total">სულ ჯამი: ' + o.total + ' ₾</div>' +
